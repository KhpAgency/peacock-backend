...
exports.paymentWebhook = asyncHandler(async (req, res, next) => {
  ...
  let query = async function ($results) {

    if ($results.payment_result.response_status !== "A") {
      const errorMessage = `Payment failed with status: ${$results.payment_result.response_status}, reason: ${$results.payment_result.response_message}`;
  ...
  paytabs.validatePayment(tranRef, query)
});
